...
const passwordChecker =  (string) => {
  // we need devide that string in 3 args:
   // - range check for the dash as a separator 
   // - letter
   // - pass
   // check for whitespaces to separte the arguments
   let rangeStr = '';
   let letter = '';
   let pw = '';
   let spaceCount = 0;
  ...
  console.log(letter);
  console.log(pw);
  // use the string trim method to remove any whitespaces
  letter = letter.trim();
  pw = pw.trim();
  // convert range into an array
  const numArr = rangeStr.split('-');
  console.log(numArr);
  numArr[0] = Number(numArr[0]);
  numArr[1] = Number(numArr[1]);
  // iterate thru the password count the number of time we have the letter 
  // check if it's in the range and return true or false
  // output: boolean
  let letterCount = 0;
  ...
  return letterCount >= numArr[0] && letterCount <= numArr[1];
};
const finalCheck = (array) => {
  return array.reduce((acc, str)=> {
    if(passwordChecker(str)) acc+=1;
    return acc;
  }, 0)
}
console.log(passwordChecker('14-17 n: nnhnnnnnnnnnnnnnhnn'));
console.log(finalCheck(input));
